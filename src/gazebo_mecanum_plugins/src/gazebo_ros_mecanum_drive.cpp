...
void GazeboRosMecanumDrive::Load(physics::ModelPtr parent, sdf::ElementPtr sdf)
{
    ...
    if (!sdf->HasElement("WheelTorque")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <WheelTorque>, "
            "defaults to %f",
    ...
    if (odometry_rate_ > 0.0) {
        ...
        if (seconds_since_last_update > (1.0 / odometry_rate_)) {
            ...
            if (wheel_accel_ == 0 ||
                ...
                joints_[LEFT_REAR]->SetParam("vel", 0,
                                             wheel_speed_instr_[LEFT_REAR]);
                joints_[LEFT_FRONT]->SetParam("vel", 0,
                                              wheel_speed_instr_[LEFT_FRONT]);
                joints_[RIGHT_FRONT]->SetParam("vel", 0,
                                               wheel_speed_instr_[RIGHT_FRONT]);
                joints_[RIGHT_REAR]->SetParam("vel", 0,
                                              wheel_speed_instr_[RIGHT_REAR]);
            }
...
void GazeboRosMecanumDrive::calkinematics(linear_vel &line_vel)
{
    ...
    line_vel.vel_x = (wheel_encoder[LEFT_REAR] + wheel_encoder[LEFT_FRONT] +
                      wheel_encoder[RIGHT_FRONT] + wheel_encoder[RIGHT_REAR]) *
                     wheel_diameter_ / 2;
    line_vel.vel_y = (wheel_encoder[LEFT_REAR] - wheel_encoder[LEFT_FRONT] +
                      wheel_encoder[RIGHT_FRONT] - wheel_encoder[RIGHT_REAR]) *
                     wheel_diameter_ / 2;
    line_vel.vel_th = (-wheel_encoder[LEFT_REAR] - wheel_encoder[LEFT_FRONT] +
                       wheel_encoder[RIGHT_FRONT] + wheel_encoder[RIGHT_REAR]) *
                      l * wheel_diameter_ / 2;
}
