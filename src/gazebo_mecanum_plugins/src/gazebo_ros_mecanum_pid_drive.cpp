...
void GazeboRosMecanumDrive::Load(physics::ModelPtr parent, sdf::ElementPtr sdf)
{
    ...
    if (!sdf->HasElement("robotNamespace")) {
        ROS_INFO_NAMED("MecanumDiffDrive",
                       "MecanumDiffDrivePlugin missing <robotNamespace>, "
                       "defaults to \"%s\"",
    ...
    if (!sdf->HasElement("commandTopic")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <commandTopic>, "
            "defaults to \"%s\"",
    ...
    if (!sdf->HasElement("odometryTopic")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <odometryTopic>, "
            "defaults to \"%s\"",
    ...
    if (!sdf->HasElement("odometryFrame")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <odometryFrame>, "
            "defaults to \"%s\"",
    ...
    if (!sdf->HasElement("robotBaseFrame")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <robotBaseFrame>, "
            "defaults to \"%s\"",
    ...
    if (!sdf->HasElement("LeftRear")) {
        ROS_WARN_NAMED("MecanumDiffDrive",
                       "MecanumDiffDrivePlugin (ns = %s) missing <LeftRear>, "
                       "defaults to \"%s\"",
    ...
    if (!sdf->HasElement("LeftFront")) {
        ROS_WARN_NAMED("MecanumDiffDrive",
                       "MecanumDiffDrivePlugin (ns = %s) missing <LeftFront>, "
                       "defaults to \"%s\"",
    ...
    if (!sdf->HasElement("RightFront")) {
        ROS_WARN_NAMED("MecanumDiffDrive",
                       "MecanumDiffDrivePlugin (ns = %s) missing <RightFront>, "
                       "defaults to \"%s\"",
    ...
    if (!sdf->HasElement("RightRear")) {
        ROS_WARN_NAMED("MecanumDiffDrive",
                       "MecanumDiffDrivePlugin (ns = %s) missing <RightRear>, "
                       "defaults to \"%s\"",
    ...
    if (!sdf->HasElement("odometryRate")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <odometryRate>, "
            "defaults to %f",
    ...
    if (!sdf->HasElement("publishWheelTF")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <publishWheelTF>, "
            "defaults to %d",
    ...
    if (!sdf->HasElement("WheelSeparationW")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <WheelSeparationW>, "
            "defaults to %f",
    ...
    if (!sdf->HasElement("WheelSeparationL")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <WheelSeparationL>, "
            "defaults to %f",
    ...
    if (!sdf->HasElement("wheelAccel")) {
        ROS_WARN_NAMED("MecanumDiffDrive",
                       "MecanumDiffDrivePlugin (ns = %s) missing <wheelAccel>, "
                       "defaults to %f",
    ...
    if (!sdf->HasElement("wheelDiameter")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <wheelDiameter>, "
            "defaults to %f",
    ...
    if (!sdf->HasElement("publishOdom")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <publishOdom>, "
            "defaults to %d",
    ...
    if (!sdf->HasElement("isRollerModel")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <isRollerModel>, "
            "defaults to %d",
    ...
    if (!sdf->HasElement("dt")) {
        ROS_WARN_NAMED("MecanumDiffDrive",
                       "MecanumDiffDrivePlugin (ns = %s) missing <dt>, "
                       "defaults to %f",
    ...
    if (!sdf->HasElement("WheelInputVelTopic")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <WheelInputVelTopic>, "
            "defaults to %s",
    ...
    if (!sdf->HasElement("WheelOutputVelTopic")) {
        ROS_WARN_NAMED(
            "MecanumDiffDrive",
            "MecanumDiffDrivePlugin (ns = %s) missing <WheelOutputVelTopic>, "
            "defaults to %s",
    ...
    if (!sdf->HasElement("WheelPID")) {
        ROS_WARN_NAMED("MecanumDiffDrive",
                       "MecanumDiffDrivePlugin (ns = %s) missing <WheelPID>, "
                       "defaults to %s",
    ...
    ROS_DEBUG_NAMED("MecanumDiffDrive", "OCPlugin (%s) has started",
                    robot_namespace_.c_str());
    ...
    ros::SubscribeOptions so_pid =
        ros::SubscribeOptions::create<gazebo_mecanum_plugins::gazebo_mecanum_plugins_pid>(
            wheel_pid, 1,
    ...
    Wheel_I_vel_pub_ =
        rosnode_->advertise<gazebo_mecanum_plugins::gazebo_mecanum_plugins_vel>(
            Wheel_I_vel, 1);
    Wheel_O_vel_pub_ =
        rosnode_->advertise<gazebo_mecanum_plugins::gazebo_mecanum_plugins_vel>(
            Wheel_O_vel, 1);
    ...
    if (this->publishWheelJointState_) {
        ...
        ROS_INFO_NAMED("MecanumDiffDrive", "%s: Advertise joint_states",
                       robot_namespace_.c_str());
    }
...
#else
        ignition::math::Pose3d poseWheel =
            joints_[i]->GetChild()->GetRelativePose().Ign();
#endif
        tf::Quaternion qt(poseWheel.Rot().X(), poseWheel.Rot().Y(),
                          poseWheel.Rot().Z(), poseWheel.Rot().W());
        tf::Vector3 vt(poseWheel.Pos().X(), poseWheel.Pos().Y(),
                       poseWheel.Pos().Z());
    ...
    if (odometry_rate_ > 0.0) {
        ...
        if (seconds_since_last_update > (1.0 / odometry_rate_)) {
            ...
            if (wheel_accel_ == 0 ||
                ...
                pid_controller_->gazebo_PID_LEFT_REAR_(joints_[LEFT_REAR],
                                                       wheel_speed_[LEFT_REAR]);
                pid_controller_->gazebo_PID_LEFT_FRONT_(
                    joints_[LEFT_FRONT], wheel_speed_[LEFT_FRONT]);
                pid_controller_->gazebo_PID_RIGHT_FRONT_(
                    joints_[RIGHT_FRONT], wheel_speed_[RIGHT_FRONT]);
                pid_controller_->gazebo_PID_RIGHT_REAR_(
                    joints_[RIGHT_REAR], wheel_speed_[RIGHT_REAR]);
                ...
                if (wheel_speed_[LEFT_REAR] > current_speed[LEFT_REAR])
                    wheel_speed_instr_[LEFT_REAR] +=
                        fmin(wheel_speed_[LEFT_REAR] - current_speed[LEFT_REAR],
                             wheel_accel_ * seconds_since_last_update);
                else
                    wheel_speed_instr_[LEFT_REAR] +=
                        fmax(wheel_speed_[LEFT_REAR] - current_speed[LEFT_REAR],
                             -wheel_accel_ * seconds_since_last_update);

                if (wheel_speed_[LEFT_FRONT] > current_speed[LEFT_FRONT])
                    wheel_speed_instr_[LEFT_FRONT] +=
                        fmin(wheel_speed_[LEFT_FRONT] - current_speed[LEFT_FRONT],
                             wheel_accel_ * seconds_since_last_update);
                else
                    wheel_speed_instr_[LEFT_FRONT] +=
                        fmax(wheel_speed_[LEFT_FRONT] - current_speed[LEFT_FRONT],
                             -wheel_accel_ * seconds_since_last_update);

                if (wheel_speed_[RIGHT_FRONT] > current_speed[RIGHT_FRONT])
                    wheel_speed_instr_[RIGHT_FRONT] += fmin(
                        wheel_speed_[RIGHT_FRONT] - current_speed[RIGHT_FRONT],
                        wheel_accel_ * seconds_since_last_update);
                else
                    wheel_speed_instr_[RIGHT_FRONT] += fmax(
                        wheel_speed_[RIGHT_FRONT] - current_speed[RIGHT_FRONT],
                        -wheel_accel_ * seconds_since_last_update);

                if (wheel_speed_[RIGHT_REAR] > current_speed[RIGHT_REAR])
                    wheel_speed_instr_[RIGHT_REAR] +=
                        fmin(wheel_speed_[RIGHT_REAR] - current_speed[RIGHT_REAR],
                             wheel_accel_ * seconds_since_last_update);
                else
                    wheel_speed_instr_[RIGHT_REAR] +=
                        fmax(wheel_speed_[RIGHT_REAR] - current_speed[RIGHT_REAR],
                             -wheel_accel_ * seconds_since_last_update);

                pid_controller_->gazebo_PID_LEFT_REAR_(
                    joints_[LEFT_REAR], wheel_speed_instr_[LEFT_REAR]);
                pid_controller_->gazebo_PID_LEFT_FRONT_(
                    joints_[LEFT_FRONT], wheel_speed_instr_[LEFT_FRONT]);
                pid_controller_->gazebo_PID_RIGHT_FRONT_(
                    joints_[RIGHT_FRONT], wheel_speed_instr_[RIGHT_FRONT]);
                pid_controller_->gazebo_PID_RIGHT_REAR_(
                    joints_[RIGHT_REAR], wheel_speed_instr_[RIGHT_REAR]);
            ...
            if (!isRollerModel_) {
                ...
                parent_->SetLinearVel(ignition::math::Vector3d(
                    cal_LineVel_.vel_x * cosf(yaw) - cal_LineVel_.vel_y * sinf(yaw),
                    cal_LineVel_.vel_y * cosf(yaw) + cal_LineVel_.vel_x * sinf(yaw),
                    0));
                parent_->SetAngularVel(
                    ignition::math::Vector3d(0, 0, cal_LineVel_.vel_th));
            }
...
{
    wheel_speed_[LEFT_REAR] =
        (x_ + y_ - rot_ * (wheel_separation_w + wheel_separation_l) / 2.0) /
        wheel_diameter_ * 2;
...
void GazeboRosMecanumDrive::calkinematics(linear_vel &line_vel)
{
    ...
    cal_LineVel_.vel_x = (wheel_encoder[LEFT_REAR] + wheel_encoder[LEFT_FRONT] +
                          wheel_encoder[RIGHT_FRONT] + wheel_encoder[RIGHT_REAR]) *
                         wheel_diameter_ / 2;
    cal_LineVel_.vel_y = (wheel_encoder[LEFT_REAR] - wheel_encoder[LEFT_FRONT] +
                          wheel_encoder[RIGHT_FRONT] - wheel_encoder[RIGHT_REAR]) *
                         wheel_diameter_ / 2;
    cal_LineVel_.vel_th = (-wheel_encoder[LEFT_REAR] - wheel_encoder[LEFT_FRONT] +
                           wheel_encoder[RIGHT_FRONT] + wheel_encoder[RIGHT_REAR]) *
                          l * wheel_diameter_ / 2;
}

void GazeboRosMecanumDrive::wheelPIDCallback(
    const gazebo_mecanum_plugins::gazebo_mecanum_plugins_pid::ConstPtr &msg)
{
