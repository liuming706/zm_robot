    ...
    node_loop_rate_(20)
{
    ...
    slow_viz_pub_ =
        nh_.advertise<visualization_msgs::Marker>("slow_range_marker", 10);
    stop_viz_pub_ =
        nh_.advertise<visualization_msgs::Marker>("stop_range_marker", 10);
    ...
    for (int i = 0; i < scan_num_; i++) {
        ...
        scan_sub_[i] = nh_.subscribe<sensor_msgs::LaserScan>(
            ScanTopicName.c_str(), 10,
            boost::bind(&ZMSafety::scanCallback, this, _1, i));
    }
...
void ZMSafety::spin()
{
    ...
    while (nh_.ok()) {
        ...
        cmd_vel_msg_.linear.x =
            unlock_safe_ == true ? init_vel_x : dist_ * init_vel_x;
        cmd_vel_msg_.linear.y =
            unlock_safe_ == true ? init_vel_y : dist_ * init_vel_y;
        cmd_vel_msg_.angular.z =
            unlock_safe_ == true ? init_vel_th : dist_ * init_vel_th;
        cmd_vel_pub_.publish(cmd_vel_msg_);
        lr.sleep();
    }
}

void ZMSafety::reconfig_callback(zm_robot_safety::zm_RobotSafetyConfig &config,
                                 uint32_t level)
{
...
void ZMSafety::scanCallback(const sensor_msgs::LaserScan::ConstPtr &msg, int idx)
{
    ...
    try {
        tfListener_.waitForTransform(
            ...
            ros::Duration(1.0));
        projector_.transformLaserScanToPointCloud("/base_link", *msg, cloud,
                                                  tfListener_);
...
{
    bool check_slow =
        (2 * abs(point.x) < slow_range_l) && (2 * abs(point.y) < slow_range_w);
    bool check_stop =
        (2 * abs(point.x) < stop_range_l) && (2 * abs(point.y) < stop_range_w);
